...
This chapter presents the gauge duality theory necessary for optimizing the PhaseLift gauge dual (PLGD) model.
Section \ref{Subsec:PLGD-models_intro} introduces the 
PhaseLift primal (PLP) and PLGD pair of models along with
relevant definitions and a brief history of gauge duality theory.
...
These results were then used in \cite{DBLP:journals/siamsc/FriedlanderM16} to develop the Gauge Dual Descent (GDD) algorithm (Algorithm \ref{Alg:PGD} in Section \ref{Subsec:PLGD_algo-algo}), a first-order algorithm for the PLP-PLGD pair.
...
The results of this chapter provide the theoretical foundation for developing the GDD algorithm for optimizing the PLGD model in Chapter \ref{Sec:PLGD_algo}.


\section{Primal-Gauge Dual Models and Background}		\label{Subsec:PLGD-models_intro}


We begin this section by stating the PhaseLift primal (PLP) and PhaseLift gauge dual (PLGD) pair of models and reviewing key definitions.
We then compare gauge duality with Lagrange duality and provide a brief history of gauge duality theory.
...
\end{array}
\end{equation}
where the linear operator $\caA$ and its adjoint $\caA^*$ are defined in (\ref{Eqn:A_definition}) and $\lambda_1(\caA^* y)$ is a function of $y \in \bbR^m$ which returns the algebraically largest eigenvalue of $\caA^*y$.
Note that in Section \ref{Subsec:PLGD-opt_conds_primal_recovery} we discuss how an approximate signal $x$ may be recovered from PLGD solution $y$.
...
\subsection{Development of Gauge Duality and Relation to Lagrange Duality}
...
Freund develops strong duality and optimality conditions for the linear pair (\ref{Eqn:PhaseLift_P_GD_linear_form}) based on polarity relationships for sets and gauge functions.  
...
\end{array}
\end{equation}
As we will see in Section \ref{Subsec:PLGD-opt_conds_primal_recovery}, the PLP-PLGD pair (\ref{Eqn:PhaseLift-P-GD}) pair is recovered from P-GD-ineq  (\ref{Eqn:PhaseLift_P_GD_inequality_form}) when we set $\kappa(X) = ||X||_1 + \delta_{(\cdot) \succeq 0}(X)$ and $\rho(y) = ||y||_2$.  
The authors of \cite{DBLP:journals/siamjo/FriedlanderMP14} develop an antipolar calculus to determine the antipolar for sets like $\{ y \ | \ \rho(Ax-b) \leq \epsilon \}$ and use this calculus rather than polarity relations to derive the pair P-GD-ineq  (\ref{Eqn:PhaseLift_P_GD_inequality_form}) and establish conditions such as those for strong duality.  Additionally, this antipolar calculus allows the authors to drop the requirement that the sets $\caX$ and $\caY$ are ray-like.
...
In this section we develop gauge duality theory for the inequality-constrained primal and gauge dual (P-GD-ineq) pair (\ref{Eqn:PhaseLift_P_GD_inequality_form}).  
Section \ref{Subsubsec:PLGD-theory_props} establishes a set of supporting propositions which are used in Section \ref{Subsubsec:PLGD-theory_thm} to prove the Gauge Duality Theorem (Theorem \ref{Thm:P-GD-inequality_pair_are_duals}).
We follow a two-track proof strategy depicted in Figure \ref{Fig:gauge_duality_thm_flowchart}, showing that the gauge duality of P-GD-ineq (\ref{Eqn:PhaseLift_P_GD_inequality_form}) is a consequence of the duality of both the nonlinear (\ref{Eqn:PhaseLift_P_GD_nonlinear_form}) and linear (\ref{Eqn:PhaseLift_P_GD_linear_form}) pairs.  
...
Finally, Section \ref{Subsubsec:PLGD-theory_weak-strong-opt} establishes weak duality, strong duality, and optimality conditions for P-GD-ineq (\ref{Eqn:PhaseLift_P_GD_inequality_form}).  
In Section \ref{Subsec:PLGD-opt_conds_primal_recovery} we will return to the PhaseLift model (\ref{Eqn:PhaseLift-P-GD}), where the optimality conditions in this section provide a method for recovery of a primal signal $x$ from a dual variable $y$.
...
This section provides a self-contained development of gauge duality for P-GD-ineq (\ref{Eqn:PhaseLift_P_GD_inequality_form}).





\subsection{Supporting Propositions} \label{Subsubsec:PLGD-theory_props}


Before presenting the Gauge Duality Theorem (Theorem \ref{Thm:P-GD-inequality_pair_are_duals} of Section \ref{Subsubsec:PLGD-theory_thm}), we present a set of supporting propositions.
The first two propositions below are used in Theorem \ref{Thm:P-GD-inequality_pair_are_duals}, (i) to show P-GD-ineq (\ref{Eqn:PhaseLift_P_GD_inequality_form}) is an instance of the nonlinear pair (\ref{Eqn:PhaseLift_P_GD_nonlinear_form}).  Since P-GD-ineq (\ref{Eqn:PhaseLift_P_GD_inequality_form}) allows for the constraint set $\caC$ to be any closed, convex set not containing the origin, we must simply establish the antipolar of the constraint set $\caC = \{ x \ | \rho(Ax-b) \leq \epsilon\}$.  We begin by showing how linear and polar transformations on $\caC$ commute under certain assumptions.
...
\subsection{Gauge Duality Theorem} \label{Subsubsec:PLGD-theory_thm}

Given the supporting propositions established in Section \ref{Subsubsec:PLGD-theory_props}, we are now prepared to prove the following theorem.
...
\subsection{Weak Duality, Strong Duality, and Optimality Conditions} \label{Subsubsec:PLGD-theory_weak-strong-opt}

With the gauge duality of the pair of models P-GD-ineq (\ref{Eqn:PhaseLift_P_GD_inequality_form}) established in Section \ref{Subsubsec:PLGD-theory_thm}, we now proceed to establish weak duality, strong duality, and optimality conditions for this pair.  These properties will play a central role in developing the GDD algorithm for optimizing the PLGD model (\ref{Eqn:PhaseLift-P-GD}), allowing us to develop termination conditions as well as a recovery method for the primal signal $x$ from a dual iterate $y$.  The weak duality property of P-GD-ineq (\ref{Eqn:PhaseLift_P_GD_inequality_form}) is a straightforward consequence of the definition of the polar of a function and inequality constraints in this model.

\begin{prop} \label{Prop:weak_duality}
\emph{(weak duality)}
Assume the primal model P-ineq (\ref{Eqn:PhaseLift_P_GD_inequality_form}) is feasible and $0 \leq \epsilon < \rho(b)$.  Then for all primal and gauge dual feasible pairs $(x, y) \in \caX \times \caY$ we have
...
\end{prop}
\begin{proof}
Since $x$ and $y$ are feasible for P-GD-ineq  (\ref{Eqn:PhaseLift_P_GD_inequality_form}), we have
...
In order to show strong duality holds for the pair P-GD-ineq (\ref{Eqn:PhaseLift_P_GD_inequality_form}), we add the assumption that $\rho^\circ$ is continuous and use the strong duality properties of the Lagrange primal and dual pair (see \cite[Section 28]{rockafellar1970convex})
...
\emph{(strong duality)}
Assume the primal model P-ineq  (\ref{Eqn:PhaseLift_P_GD_inequality_form}) is feasible, $\rho^\circ$ is continuous, and $0 \leq \epsilon < \rho(b)$.  Then P-ineq admits an optimal variable $x_\star$ and
...
We close this section by establishing the optimality conditions of the pair P-GD-ineq  (\ref{Eqn:PhaseLift_P_GD_inequality_form}) which are primarily a consequence of the strong and weak duality results above.


\begin{prop} \label{Prop:opt_conds}
\emph{(optimality conditions)}
If the primal model P-ineq (\ref{Eqn:PhaseLift_P_GD_inequality_form}) is feasible, $\rho^\circ$ is continuous, and $0 \leq \epsilon < \rho(b)$, then the pair $(x, y) \in \caX \times \caY$ is primal-dual optimal if and only if the following conditions hold:
...
This section establishes optimality conditions and a primal signal recovery strategy for the PLGD model (\ref{Eqn:PhaseLift-P-GD}), two essential tools for developing the Gauge Dual Descent (GDD) algorithm presented in Chapter \ref{Sec:PLGD_algo}.  
We see first that the duality of the PLP-PLGD pair (\ref{Eqn:PhaseLift-P-GD}) is a direct result of the duality previously established for the pair P-GD-ineq (\ref{Eqn:PhaseLift_P_GD_inequality_form}).  
...
We also present a more general optimality condition which applies to the GDD algorithm.
...
We may also set $\rho(y) := ||y||_2$, which has the polar (or dual norm) $\rho^\circ(y) = ||y||_* = ||y||_2$.  Thus PLP has the form of P-ineq (\ref{Eqn:PhaseLift_P_GD_inequality_form})
\begin{equation*} 			
\begin{array}{ll}
...
Given the optimality and primal recovery properties established above, we now proceed to develop the Gauge Dual Descent (GDD) algorithm, a first-order algorithm for optimizing the PLGD model (\ref{Eqn:PhaseLift-P-GD}) and recovering a primal signal $x$ from the PLGD solution $y$.
